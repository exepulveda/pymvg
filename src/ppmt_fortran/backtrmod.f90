!Module for back-transforming a Gaussian value to its original distribution 
!units in gslib type programs
! Authors: John Manchuk, 2010
!          Ryan Barnett, 2012
...
! backtr modified from the original gslib form
    ...
    real*8 function backtr ( vrgs,vr,vrg,zmin,zmax,ltail,ltpar,utail,utpar ) result ( z )
!-----------------------------------------------------------------------
!
...
!
!-----------------------------------------------------------------------
      real*8, intent (in) :: vrgs, vr(:), vrg(:), zmin, zmax, ltpar, utpar
...
!
      if(vrgs.le.vrg(1)) then
            z = vr(1)
            if(ltail.eq.1) then
                  z = powint(0.d0,snorm_cdf(vrg(1)),zmin,vr(1),snorm_cdf(vrgs),1.d0)
            else if(ltail.eq.2) then
                  cpow   = 1.d0 / ltpar
                  z = powint(0.d0,snorm_cdf(vrg(1)),zmin,vr(1),snorm_cdf(vrgs),cpow)
            endif
!
...
!
            ...
            z = vr(nt)
            if(utail.eq.1) then
                  z = powint(snorm_cdf(vrg(nt)),1.d0,vr(nt),zmax,snorm_cdf(vrgs),1.d0)
            else if(utail.eq.2) then
                  cpow   = 1.d0 / utpar
                  z = powint(snorm_cdf(vrg(nt)),1.d0,vr(nt),zmax,snorm_cdf(vrgs),cpow)
            else if(utail.eq.4) then
                  lambda = (vr(nt)**utpar)*(1.d0-snorm_cdf(vrg(nt)))
                  z = (lambda/(1.d0-snorm_cdf(vrgs)))**(1.d0/utpar)
            endif
      else
!
...
!
            ...
            j = max(min((nt-1),j),1)
            z = powint(vrg(j),vrg(j+1),vr(j),vr(j+1),vrgs,1.d0)
      endif
      return
     end function backtr
